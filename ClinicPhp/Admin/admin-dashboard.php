...
<div id="patientDropdownList" class="custom-dropdown-list">
    ...
    if (!$connTemp->connect_error) {
        // --- FIXED QUERY: Only fetch 'approved' users ---
        $patSql = "SELECT surname, firstName, gender, age 
                   FROM users 
        ...
        if ($patResult && $patResult->num_rows > 0) {
            ...
            echo "<div class='dropdown-item' style='pointer-events: none; color: #999;'>No approved patients found</div>";
        }
        $connTemp->close();
    }
    ?>
...
<div id="monthlyReport" class="content-section" style="display: none;">
    ...
    <div class="charts-row" style="display: flex; gap: 20px; width: 100%; margin-bottom: 30px;">
        
        <div class="chart-box" style="flex: 1; background: #fff; padding: 15px; border-radius: 8px; box-shadow: 0 2px 5px rgba(0,0,0,0.1);">
            <h3>Medicines Released</h3>
        ...
        <div class="chart-box" style="flex: 1; background: #fff; padding: 15px; border-radius: 8px; box-shadow: 0 2px 5px rgba(0,0,0,0.1);">
            <h3>Medicines Expired (This Month)</h3>
            <div style="height: 300px; position: relative;">
    ...
    <div class="lists-container" style="display: flex; gap: 20px;">
        <div class="list-box" style="flex: 1; background: white; padding: 15px; border-radius: 8px; box-shadow: 0 2px 5px rgba(0,0,0,0.1);">
...
<div id="patientLogsModal" class="modal">
    <div class="modal-content" style="max-width: 900px;">
        <span class="close" onclick="document.getElementById('patientLogsModal').style.display='none'">&times;</span>
        <h2 id="logsPatientName">Medical History</h2>
            ...
            <table class="report-table" style="width: 100%; border-collapse: collapse; margin-top: 15px;">
                <thead>
                    <tr style="background: #f8f9fa; text-align: left;">
                        ...
                        <th style="padding: 10px;">Vitals (BP/HR/Temp)</th>
                        <th style="padding: 10px;">Measurements (H/W/BMI)</th>
                        <th style="padding: 10px;">Findings</th>
                    </tr>
                </thead>
                <tbody id="patientLogsTableBody">
                    </tbody>
            </table>
        </div>
    </div>
...
<div id="warningApproveModal" class="modal">
    <div class="modal-content" style="max-width: 500px; border-top: 5px solid #e74c3c;">
        <span class="close" onclick="document.getElementById('warningApproveModal').style.display='none'">&times;</span>
        
        <h2 style="color: #c0392b;"><i class="fas fa-exclamation-triangle"></i> Review Needed</h2>
        
        <p style="font-size: 16px; color: #555; margin: 20px 0;">
            This user has <strong>NOT updated their profile yet</strong>.
        </p>
        <p style="color: #666; font-size: 14px; background: #fdf2f2; padding: 10px; border-radius: 5px;">
            Are you sure you want to approve this account anyway? <br>
            It is recommended to wait until they fill in their details.
        </p>

        <div style="margin-top: 25px; text-align: right; display: flex; justify-content: flex-end; gap: 10px;">
            <button class="btn cancel" onclick="document.getElementById('warningApproveModal').style.display='none'">Cancel</button>
            <button id="confirmApproveBtn" class="btn" style="background-color: #c0392b;">Approve Anyway</button>
        </div>
    </div>
</div>

<div id="standardApproveModal" class="modal">
    <div class="modal-content" style="max-width: 500px; border-top: 5px solid #27ae60;">
        <span class="close" onclick="document.getElementById('standardApproveModal').style.display='none'">&times;</span>
        
        <h2 style="color: #27ae60;"><i class="fas fa-check-circle"></i> Approve Account</h2>
        
        <p style="font-size: 16px; color: #555; margin: 20px 0;">
            This user has completed their profile setup.
        </p>
        <p style="color: #666;">
        ...
        <div style="margin-top: 25px; text-align: right; display: flex; justify-content: flex-end; gap: 10px;">
            <button class="btn cancel" onclick="document.getElementById('standardApproveModal').style.display='none'">Cancel</button>
            <button id="btnStandardConfirm" class="btn" style="background-color: #27ae60;">Confirm Approval</button>
        </div>
    </div>
</div>
